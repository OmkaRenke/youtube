...
const categories = [
    ...
    { name: "Trending", icon: <MdLocalFireDepartment size={"20px"}/>, type: "category" },
    ...
    { name: "Gaming", icon: <IoGameControllerSharp size={"20px"} />, type: "category" },
    { name: "News", icon: <ImNewspaper size={"20px"}/>, type: "category" },
    { name: "Sports", icon: <GiDiamondTrophy size={"20px"}/>, type: "category" },
    { name: "Learning", icon: <RiLightbulbLine size={"20px"}/>, type: "category" },
    {
        name: "Fashion & beauty",
    ...
    { name: "Report History", icon: <AiOutlineFlag size={"20px"}/>, type: "menu" },
    { name: "Help", icon: <FiHelpCircle size={"20px"}/>, type: "menu" },
    { name: "Send feedback", icon: <RiFeedbackLine size={"20px"}/>, type: "menu" }
];
...
const LeftNav = () => {
  ...
  <div className={`maincontainerBox ${mobileMenu ? "mobilemenuview":""}`}>
      {categories.map((item,index)=><LeftNavbarMenuItem key={index}
        ...
        data={item}
        handleCategorychange={()=>{handleCategorychange(item.name,item.type);navigate('/')}}
